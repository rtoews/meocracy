...
<ul data-role='listview' data-inset='false' data-theme='a'  data-divider-theme='a'>
    <li>
        <img src="<?php echo $region_logo; ?>"/>
        <?php echo $city->masthead_city_of(); ?><br/>
        <?php echo $city->masthead_city_state(); ?>
    </li>
...
if (!empty($announcements)) {
    foreach ($announcements as $ann) {
        if ($ann->feedback_submitted($user_id)) {
            $output  = '<li>' . HTML_EOL;
            $output .= '    <a href="/feedback_response.php?id=' . $ann->id() . '">' . HTML_EOL;
            $output .= '        <img src="' . $ann->get_image_src() . '" class="ui-li-thumb"/>' . HTML_EOL;
            $output .= '        ' . $ann->heading() . HTML_EOL;
            $output .= '    </a>' . HTML_EOL;
            $output .= '</li>' . HTML_EOL;
        }
        else {
            $output  = '<li>' . HTML_EOL;
            $output .= '    <a href="/announcement.php?id=' . $ann->id() . '">' . HTML_EOL;
            $output .= '        <img src="' . $ann->get_image_src() . '" class="ui-li-thumb"/>' . HTML_EOL;
            $output .= '        ' . $ann->heading() . HTML_EOL;
            $output .= '    </a>' . HTML_EOL;
            $output .= '</li>' . HTML_EOL;
...
if (!empty($legislation)) {
    foreach ($legislation as $leg) {
        if ($leg->feedback_submitted($user_id)) {
            $output  = '<li>' . HTML_EOL;
            $output .= '    <a href="/feedback_response.php?id=' . $leg->id() . '">' . HTML_EOL;
//            $output .= '        <img src="' . $leg->get_image_src() . '" class="ui-li-thumb"/>' . HTML_EOL;
            ...
            $output  = '<li>' . HTML_EOL;
            $output .= '    <a href="/legislation.php?id=' . $leg->id() . '">' . HTML_EOL;
//            $output .= '        <img src="' . $leg->get_image_src() . '" class="ui-li-thumb"/>' . HTML_EOL;
            $output .= '        ' . $leg->title() . HTML_EOL;
            $output .= '    </a>' . HTML_EOL;
            $output .= '</li>' . HTML_EOL;
