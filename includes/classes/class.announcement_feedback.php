...
class Announcement_Feedback extends DBInterface
{
    ...
    {
        $sql = sprintf("SELECT feedback_id FROM announcement_feedback WHERE user_id=%d AND announcement_id=%d", $this->user_id(), $this->_issue_id);
    ...
    {
        $feedback = array();
        $sql = sprintf("SELECT feedback_id FROM announcement_feedback WHERE announcement_id=%d", $this->_issue_id);
    ...
    {
        $feedback = array();
        $sql = sprintf("SELECT feedback_id FROM announcement_feedback WHERE announcement_id=%d AND comments > ''", $this->_issue_id);
        $data = db()->Get_Table($sql);
        if (!empty($data)) {
            foreach ($data as $row) {
                $feedback[] = new Announcement_Feedback($row['feedback_id']);
